# Evenza - Full-Stack Event Booking Platform
...
- `npm run dev` starts both `backend` and `frontend` together using `concurrently`.
- `npm run dev:all` installs dependencies (backend + frontend) and starts both in one command.
...
_(frontend will run on a local port provided by Vite, typically `http://localhost:5173`)_
